...
enqueue_account_push(account_id)
```

- [ ] **步骤 4：移除注册后的自动 OTP 登录**
...
预期：现有 133 项及新增测试全部 PASS，没有失败或错误。

- [ ] **步骤 3：在不重启服务的情况下验证快速测活**

选择一个已有账号调用快速测活 API；确认不产生新 OTP 邮件、Token 检查返回成功、推送保持幂等，且总并发快照峰值不超过 2。

- [ ] **步骤 4：受控执行一次完整 OTP 登录实测**
