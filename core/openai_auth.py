...
def register_user(
    ...
    logger.info("[步骤7] 提交邮箱和密码：%s", redact_email(email))
    resp = session.post(url, headers=headers, data=body)
    if resp.status_code != 200:
        body_text = resp.text or ""
        logger.error(
            "[步骤7] user/register 失败 status=%s body_len=%s body_fp=%s",
            resp.status_code, len(body_text), hashlib.sha1(body_text.encode("utf-8", "replace")).hexdigest()[:12],
        )
        resp.raise_for_status()
    data = resp.json()
    return data
...
def validate_email_otp(session: BrowserSession, code: str, sentinel_header: str | None = None, so_header: str | None = None) -> dict:
    ...
    logger.info("[步骤10] 提交邮箱验证码，code_len=%s", len(str(code or "")))
    resp = session.post(url, headers=headers, data=body)

    if resp.status_code != 200:
        logger.error(
            "[步骤10] 请求失败，status=%s error_code=%s response_len=%s",
            resp.status_code,
            _extract_error_code(resp) or "-",
    ...
    logger.info("[步骤10] 验证响应结构 keys=%s", sorted(str(key) for key in data.keys()))
    return data
...
def create_account(session: BrowserSession, name: str, birthday: str, sentinel_header: str, so_header: str = None) -> dict:
    ...
    if resp.status_code != 200:
        logger.error(
            "[步骤12] 请求失败，status=%s error_code=%s response_len=%s",
            resp.status_code,
            _extract_error_code(resp) or "-",
            len(resp.content or b""),
        )
