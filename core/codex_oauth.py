...
def _complete_mfa_if_required(session: BrowserSession, email: str, result: dict | None) -> dict:
    ...
    logger.info("[Codex] Auth 要求 MFA，开始提交账号 TOTP：%s factor_id=%s", redact_email(email), factor_id)
...
def _try_password_mfa_login(
    ...
    logger.info("[Codex] 账号存在注册密码，优先走密码登录：%s", redact_email(email))
    ...
    if _is_email_otp_step(result) or page_type in {"email_verification", "email_otp_send"}:
        logger.info("[Codex] 密码登录后服务端仍要求邮箱 OTP，切换到邮箱 OTP：%s", redact_email(email))
        return "email_otp", None, result

    callback_url = _follow_login_continue(session, continue_url, state) if continue_url else None
    logger.info("[Codex] 密码/MFA 登录链已完成，继续 Codex workspace/callback：%s", redact_email(email))
...
def run_codex_oauth(
    ...
    if transport == "page":
        ...
        from core.cloakbrowser_driver import account_fingerprint_seed as _seed_for

        page_driver, _opened = build_cloak_driver(
            proxy=proxy or None,
            proxy_selection=proxy_selection,
            fingerprint_seed=_seed_for(email),
        )
    ...
    try:
        ...
        if _is_phone_step(auth_result):
            logger.info("[Codex] Auth 明确要求手机号验证，开始接码：%s", redact_email(email))
