    ...
    for attempt in range(1, max_otp_attempts + 1):
        try:
            if current_otp is None:
                ...
                current_otp = otp_wait_session.wait(email, after_ts=otp_after_ts, email_source=email_source)
...
def check_account_liveness(
    ...
    *,
    ...
    try:
        ...
        if existing_access_token and not has_totp:
            ...
            try:
                ...
                if not _is_retryable_network_error(reauth_exc):
                    raise
                # reauth/callback 的同会话阶段重试已经耗尽。继续复用熔断的 Cookie Jar
                # 没有意义；参考 plus 纯协议注册，建立干净会话并按
            ...
            # 无 AT / 已开 TOTP 的账号统一走完整 Web 登录链：登录页 document →
            # Providers/Session/CSRF/Session → Signin → Authorize → 密码/邮箱 OTP →
            # MFA(如有) → OAuth callback。函数内部仍复用我方 _login_via_password_or_otp。
            logger.info(
        ...
        if code:
            logger.warning("[查活] 已废号：%s %s", redact_email(email), code)
